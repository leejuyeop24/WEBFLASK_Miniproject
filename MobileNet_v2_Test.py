...
# --------------------------- #
# 2. 모델 불러오기 (MobileNet_v2, 5개 클래스)
# --------------------------- #
model = models.mobilenet_v2(pretrained=False)

# MobileNet_v2는 classifier[-1] 수정
num_features = model.classifier[-1].in_features
model.classifier[-1] = nn.Linear(num_features, 5)  # ✅ 5진 분류

# 학습된 모델 가중치 로드
model.load_state_dict(torch.load('mobilenetv2_best_model.pth', map_location=device))
